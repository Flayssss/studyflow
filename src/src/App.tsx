...
interface ExamFolder {
  ...
  approvalProbability: number; // 1 to 100 calculated estimate
...
const DEFAULT_EXAMS: ExamFolder[] = [
  ...
  },
  {
    ...
    salary: 'R$ 16.500,00',
    vagas: '42',
    examSystemDescription: 'Múltipla escolha de altíssima complexidade gramatical e lógica com enunciados interpretativos e jurisprudenciais complexos.',
    registrationStart: '10/09/2026',
    registrationEnd: '25/10/2026',
    examDate: '06/12/2026',
    overlapAnalysis: 'Compatibilidade de 44% com seu progresso. Exige esforço maciço e foco total em Regimento Interno da CLDF e Processo Legislativo.',
    bancaStyleExplanation: 'A FGV é famosa por provas de português extremamente subjetivas e jurisprudências de tribunais superiores em Direito.',
    approvalProbability: 35,
    syllabus: [
      {
        subjectName: 'Processo Legislativo',
        topics: [
          { topicName: 'Iniciativa de leis e rito das comissões', completed: false, reviewed: false, exercisesDone: false, questionsCount: 0, correctAnswers: 0, difficulty: 5, lastStudied: 'Nunca', revisionsCount: 0 },
          { topicName: 'Espécies normativas (Emendas, Leis Complementares)', completed: false, reviewed: false, exercisesDone: false, questionsCount: 0, correctAnswers: 0, difficulty: 4, lastStudied: 'Nunca', revisionsCount: 0 }
        ]
      },
      {
        subjectName: 'Direito Constitucional',
        topics: [
          { topicName: 'Direitos e Garantias Fundamentais (Art. 5º ao 17)', completed: true, reviewed: true, exercisesDone: true, questionsCount: 60, correctAnswers: 52, difficulty: 2, lastStudied: 'Ontem', revisionsCount: 4 },
          { topicName: 'Organização dos Poderes e Controle de Constitucionalidade', completed: false, reviewed: false, exercisesDone: false, questionsCount: 0, correctAnswers: 0, difficulty: 5, lastStudied: 'Nunca', revisionsCount: 0 }
        ]
      }
    ]
  }
...
const INITIAL_SUBJECTS: Subject[] = [
  ...
  { id: '4', name: 'Raciocínio Lógico', progress: 25, accuracy: 55, color: '#f59e0b', mastery: 'Iniciado' },
];
...
const INITIAL_LOGS: QuestionLog[] = [
  ...
  { id: 'l3', subject: 'Direito Constitucional', total: 10, correct: 6, xpGained: 90, timestamp: 'Há 3 dias' },
];
...
const INITIAL_FLASHCARDS: Flashcard[] = [
  ...
  },
  {
    ...
    front: 'A autarquia possui personalidade jurídica de direito privado e autonomia administrativa e financeira?',
    back: 'ERRADO. As autarquias possuem personalidade jurídica de direito público (são extensões do próprio Estado criadas por lei específica).',
    explanation: 'Explicação da IA: Empresa Pública e Sociedade de Economia Mista são de Direito Privado. Autarquias são de Direito Público!',
    difficultyRating: 4,
    easeFactor: 1.8,
    repetitions: 2,
    intervalDays: 2,
...
const WizardAvatar = ({ 
  ...
  const colorPercentage = Math.min(100, level);
    ...
    <div className="relative flex flex-col items-center group">
        ...
        <div className="relative w-36 h-36 rounded-full overflow-hidden border-2 border-indigo-500/30 shadow-[0_0_25px_rgba(99,102,241,0.2)] bg-slate-950 flex items-center justify-center">
          {imageUrl ? (
            <div className="relative w-full h-full">
              ...
              />
              <div 
                className="absolute inset-0 bg-transparent mix-blend-normal pointer-events-none transition-all duration-500"
                style={{
                  backdropFilter: `grayscale(${Math.max(0, 100 - colorPercentage)}%)`,
                  opacity: Math.max(0, (100 - colorPercentage) / 100)
                }}
              />
            </div>
          ) : (
      ...
      <div className="mt-4 text-center">
        <span className="px-3 py-1 bg-indigo-500/10 border border-indigo-500/20 rounded-full text-[9px] font-black text-indigo-400 uppercase tracking-tighter">
          {level < 10 ? 'Iniciado Arcano' : level < 40 ? 'Mago de Elite' : 'Arquimago Supremo'}
        </span>
...
export default function App() {
  ...
  const handleToggleSyllabusMetric = (
    ...
    setExamFolders(prevExams => 
      prevExams.map(exam => {
        ...
        // Dynamic Approval Probability Re-calculation
        let totalCount = 0;
        let checkedCount = 0;
  ...
  const handleTriggerAiEditalAnalysis = async () => {
    ...
    try {
      // Direct call using prompt schemas
      ...
      // Offline simulation fallback block
      ...
      setAiNotification({ message: 'TJDFT extraído e adicionado via Mapeador Inteligente Offline!', type: 'success' });
    } finally {
  ...
  const prefillSamplePreset = (type: 'sedes' | 'der' | 'cldf') => {
    const textPresets = {
      sedes: "CONCURSO SECRETARIA DE DESENVOLVIMENTO SOCIAL (SEDES-DF). Banca IADES. Prova dia 26/08/2026. Inscrições de 10/05/2026 a 15/06/2026. Salário R$ 5.480,00. Vagas: 120 + CR. Matérias: Compreensão de texto, ortografia, concordância verbal, atos administrativos, organização do Estado, Lei Orgânica da Assistência Social (LOAS).",
      der: "DEPARTAMENTO DE ESTRADAS DE RODAGEM DO DF (DER-DF). Banca IADES. Prova dia 25/10/2026. Salário R$ 7.200,00. Vagas: 85. Conteúdo: Legislação de Trânsito específica de estradas, CTB, sinalização e Português instrumental.",
                  ...
                  <div className="lg:col-span-5 space-y-6">
                    ...
                    <GlassCard className="p-6">
                      ...
                      <p className="text-xs text-slate-400 leading-relaxed mb-4">
                        Cole o conteúdo programático, as datas de inscrição ou o PDF do edital abaixo. A IA do site gerará automaticamente um novo edital verticalizado, cronograma e comparador de similaridade.
                      </p>
                      ...
                      <div className="space-y-3">
                        <textarea
                          ...
                          placeholder="Cole o trecho do edital ou regulamento do novo concurso aqui..."
